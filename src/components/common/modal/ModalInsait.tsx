import Image from "next/image"
import { TCardInsait } from "../cards/mainPageCards/InsaitCard"

export default function ModalInsait({ header, name, text, open, setModalDataInsait }: TCardInsait) {
    return (
        <div onClick={() => setModalDataInsait({
            header: '',
            name: '',
            text: '',
            open: false,
        })}
            className={`p-10 w-screen h-screen bg-tr04 z-[100] backdrop-blur-sm fixed top-0 left-0 ${open ? 'flex' : 'hidden'} justify-center items-center`}
        >
            <div className="max-h-full w-[600px] max-w-[600px] min-w-[330px] bg-orange-class px-5 pt-4 pb-6 text-white">
                
                <h4 className="text-heading-ss-bold flex justify-between">
                    {header}
                    <Image height={27} width={27} src={'/close.svg'} alt='person' />
                </h4>
                <div className="py-3 flex gap-1 items-center">
                    <Image className="rounded-4 h-[27px] w-[27px] object-cover" height={27} width={27} src={'/delete/person.jpeg'} alt='person' />
                    <p className="text-13_600">{name}</p>
                </div>
                <div className="max-h-[400px] py-1 overflow-y-scroll scroll-hidden text-12_500">
                    {text}
                </div>
            </div>
        </div>

    )
}